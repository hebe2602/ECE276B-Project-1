            ...
            for o in orientation:
                for j in range(key_door_info_size):
                    ...
                    if w_i == goal_loc[0] and h_i == goal_loc[1]:
                        green_square_location_ind.append(len(state_space_list) - 1)
                   

                    if state_i['agent_pos'] == start_pos_agent and (state_i['agent_or'] == info['init_agent_dir']).all():
                        if state_i['door'] == 0 and state_i['have_key'] == 0:
                            start_ind = len(state_space_list)-1
...
def next_action(a,b,key_pos,door_pos):
    ...
    for action in possible_actions:
        ...
        if action == UD:
            if ([pos + ori for pos, ori in zip(a['agent_pos'], a['agent_or'])] == door_pos).all():
                if a['agent_pos'] == b['agent_pos'] and a['agent_or'] == b['agent_or'] and a['have_key'] == 1:
        ...
        if action == TR:
            if a['agent_pos'] == b['agent_pos'] and a['have_key'] == b['have_key'] and a['door'] == b['door']:
                ...
                #next_or = np.dot(np.array((a['agent_or'])),arr)
        ...
        if action == MF:
            if a['agent_or'] == b['agent_or'] and [pos + ori for pos, ori in zip(a['agent_pos'], a['agent_or'])] == b['agent_pos']:
                if a['have_key'] == b['have_key'] and a['door'] == b['door']:
                    ...
                    if (b['agent_pos'] == door_pos).all() and b['door'] == 0:
                        return 5
                    return MF
...
def doorkey_problem(env,info):
    ...
    while len(open_list) > 0:
        ...
        i = open_list.pop(-1) #DFS variation
